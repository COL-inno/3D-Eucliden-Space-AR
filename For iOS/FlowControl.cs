...
public class FlowControl : MonoBehaviour
{
        ...
        switch(sceneNumber) {
            ...
            case 11 : 
                ...
                if (!dialogPlayed[sceneNumber]) {
                    ...
                    dashPoint(point.transform.position);
                    //bool bPosReached1 = bPos.x - point.transform.position.x <= 0.0005f;
            ...
            case 12 :
                ...
                if (!dialogPlayed[sceneNumber]) {
                    ...
                    bool ABCReached = pointABCPos.y - point.transform.position.y <= 0.0005f;
                    if (!ABCReached) {
    ...
    void showZaxis (Vector3 tapPos) {
        ...
        conez.transform.position = zaxiscon.transform.position + dest + conez.transform.forward * axisThickness;
    }
    ...
    void dashPoint (Vector3 pointPos, bool isDashW) {
        Vector3 proj = new Vector3 (pointPos.x - zero.transform.position.x, 0, pointPos.z - zero.transform.position.z);
        Vector3 projXY = zero.transform.position + proj;
        Vector3 xPos = Vector3.Project(proj, zero.transform.forward) + zero.transform.position;
        Vector3 yPos = Vector3.Project(proj, zero.transform.right * -1) + zero.transform.position;
